...
class Transfer extends Base_model {
	...
	public function db_save()
	{
		...
		{ // Check for valid new transfer status
			if( $this->_check_items() )
			{
				...
				{
					die( 'Invalid transfer status for new record');
				}
	...
	public function receive()
	{
		...
		{
			die( 'Cannot receive transfer. Only approved transfers or transfers from an external source can be received.');
		}

		// Only destination store can receive the transfer
		if ( $ci->session->current_store_id != $this->destination_id )
		{
			die( 'Current store is not authorized to receive the transfer.' );
		}
