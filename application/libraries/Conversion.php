...
class Conversion extends Base_model {
	...
	public function _transact_conversion()
	{
		...
		if( $source_inventory && $target_inventory )
		{
			...
			if( $this->source_quantity > $this->target_quantity )
			{
				$conversion_category = $Category->get_by_name( 'Pack' );
			}
			elseif( $this->source_quantity < $this->target_quantity )
			{
				$conversion_category = $Category->get_by_name( 'Unpack' );
			}
			else
			{
				$conversion_category = $Category->get_by_name( 'Conversion' );
			}

			$source_inventory->transact( TRANSACTION_CONVERSION_FROM, ( $this->source_quantity * -1 ), $transaction_datetime, $this->id, NULL, $conversion_category->get( 'id' ) );
			$target_inventory->transact( TRANSACTION_CONVERSION_TO, $this->target_quantity, $transaction_datetime, $this->id, NULL, $conversion_category->get( 'id' ) );
			$ci->db->trans_complete();
		}
		else
		{
