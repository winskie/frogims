		...
		<table class="table table-condensed">
			...
			<tbody>
				<tr ng-repeat="row in moppingItem.items"
					...
					<td class="text-right">{{ row.mopped_quantity | number }}</td>
					<td class="text-right">{{ row.mopped_base_quantity | number }}</td>
				...
				<tr ng-if="moppingItem.items.length == 0">
					<td colspan="{{ data.editMode == 'view' ? 8 : 9 }}" class="text-center bg-warning">
