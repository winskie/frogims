...
app.controller( 'DashboardController', [ '$scope', '$filter', '$http', '$state', '$stateParams', 'baseUrl', 'session', 'notifications', 'utilities',
    function( $scope, $filter, $http, $state, $stateParams, baseUrl, session, notifications, utilities )
    {
        ...
        $scope.history = {
		        chart: null,
		        config: {
						title: { text: 'Activity for the past 24 hours' },
						xAxis: {
						        type: 'datetime',
						        title: { text: 'time' },
       ...
       $scope.redrawHistory = function()
            {
                $http({
                    method: 'GET',
                    url: baseUrl + 'index.php/api/v1/stores/' + session.data.currentStore.id +  '/inventory_history',
                    params: {
                        start: $filter( 'date' )( $scope.startDate, 'yyyy-MM-dd HH:mm:ss')
                    }
                }).then(
                    function( response )
                    {
                        var currentSeries = $scope.history.chart.series;
                        var series = processData( response.data.data );
                        ...
                        {
                            $scope.history.chart.addSeries( series[j], false );
                        }

                        $scope.history.chart.redraw();
                    },
                    function( reason )
                    {
